...
const ResetPassword = () => {
  ...
  const { themeMode, axios, navigate, backend } = useContext(AppContext)
  ...
  //     const { data } = await axios.post(backend + '/api/auth/send-reset-otp',email);
  //     if (data.success) {
  ...
  async function sendOtpToServer(e) {
    try {
      axios.defaults.withCredentials = true;
      e.preventDefault();
      const OTP = otp.join('');
  ...
  async function sendVerificationPasswordOtp(e) {
    try {
      e.preventDefault();
  ...
  const display = isEmailSent && 'justify-center';

  return (
    <div className={`${bgColor} min-h-screen flex ${display} items-center`}>
      {!isEmailSent && <form onSubmit={sendVerificationPasswordOtp} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] rounded-lg shadow-xl border border-gray-200 bg-white">
        ...
        <button className='w-full py-2  rounded-2xl cursor-pointer bg-gradient-to-br from-blue-600 via-blue-200 text-lg text-gray-800 hover:bg-blue-500 font-semibold'>Send OTP</button>
        <button onClick={() => navigate(-1)} className='w-full py-2  rounded-2xl cursor-pointer bg-gradient-to-br from-blue-600 via-blue-200 text-lg text-gray-800 hover:bg-blue-500 font-semibold'>Go Back</button>
      </form>}
      {isEmailSent && <div className="bg-white p-6 rounded-xl w-full max-w-md">
        <h2 className="text-2xl font-semibold text-center text-gray-800 mb-6"><span className='text-blue-500'>Enter</span> OTP</h2>
        <form onSubmit={sendOtpToServer} className="space-y-6">
          ...
          <div className="w-full ">
          <p>New Password</p>
          <input onChange={(e) => setPassword(e.target.value)} value={password} placeholder="enter your  New Password" className="border border-gray-200 rounded w-full p-2 mt-1 outline-blue-500" type="password" required />
        </div>
        ...
        {/* <p className="text-center text-sm text-gray-500 mt-6">
          Didn't receive the code? <span onClick={sendResetOtp} className='text-blue-600 cursor-pointer'>Resend</span>
        </p> */}
      </div>}
